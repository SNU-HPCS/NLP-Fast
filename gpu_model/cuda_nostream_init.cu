...
#include "cuda_nostream_init.cuh"
...
static int cuda_stream_init(Params *params, gpu_cuda_context_t *gpu_context) {
	...
	gpu_context->cublas_handles = (cublasHandle_t*)malloc(sizeof(cublasHandle_t));
	if ((cublas_rc = cublasCreate(gpu_context->cublas_handles)) != CUBLAS_STATUS_SUCCESS)  {
		fprintf(stderr, "cublasCreate (cublas_rc: %d)\n", cublas_rc);
		goto err;
	}
...
int cuda_bert_main(Params *params, BERT_State *bert_state, gpu_cuda_context_t *gpu_context){
	...
	for (int layer_idx = 0; layer_idx < num_layer; layer_idx++) {
		...
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_QLW][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_QLW][0],
				hidden_size * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		// copy weight_QLB
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_QLB][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_QLB][0],
				seq_length * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemm(*gpu_context->cublas_handles, CUBLAS_OP_N, CUBLAS_OP_N,
										 seq_length, hidden_size, hidden_size,
										 &alpha, gpu_context->d_input[batch_idx], seq_length,
										 &gpu_context->d_weight[layer_idx][WEIGHT_QLW][0], hidden_size,
										 &beta, &gpu_context->d_buf_query[batch_idx][0], seq_length)) != CUBLAS_STATUS_SUCCESS) {
				fprintf(stderr, "[Q_GEN] cublasSgemm (cublas_rc: %d)\n", cublas_rc); goto err;
			}

			if ((cublas_rc = cublasSaxpy(*gpu_context->cublas_handles, seq_length * hidden_size, &alpha,
										 &gpu_context->d_weight[layer_idx][WEIGHT_QLB][0], 1,
										 &gpu_context->d_buf_query[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		// copy weight_KLW
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_KLW][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_KLW][0],
				hidden_size * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		// copy weight_KLB
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_KLB][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_KLB][0],
				seq_length * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemm(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
										 seq_length, hidden_size, hidden_size,
										 &alpha, gpu_context->d_input[batch_idx], seq_length,
										 &gpu_context->d_weight[layer_idx][WEIGHT_KLW][0], hidden_size,
										 &beta, &gpu_context->d_buf_key[batch_idx][0], seq_length)) != CUBLAS_STATUS_SUCCESS) {
			...
			if ((cublas_rc = cublasSaxpy(gpu_context->cublas_handles[0], seq_length * hidden_size, &alpha,
										 &gpu_context->d_weight[layer_idx][WEIGHT_KLB][0], 1,
										 &gpu_context->d_buf_key[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemmStridedBatched(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_T,
													   ...
													   &alpha, &gpu_context->d_buf_query[batch_idx][0],
													   seq_length, seq_length * head_size,
													   &gpu_context->d_buf_key[batch_idx][0],
													   seq_length, seq_length * head_size,
													   &beta, &gpu_context->d_buf_score[batch_idx][0],
													   seq_length, seq_length * seq_length, num_heads)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			dim3 dimGrid(num_heads * seq_length * seq_length / params->thread_block_size);

			g_score_norm_layer_mask<<<dimGrid, dimBlock, 0>>>(
					&gpu_context->d_buf_score[batch_idx][0], score_norm_factor,
					gpu_context->d_attention_mask[batch_idx], seq_length, seq_length, num_heads);
		}
//		sync_all_buf_to_host(bert_state, gpu_context);
...
//					float alpha = 1., beta = 0.;
			...
			/// exp_sum
			cudaMemset(&gpu_context->d_buf_expsum[batch_idx][0],
							0, num_heads * seq_length * 1 * sizeof(float));
			g_exp_sum<<<dimGrid, dimBlock, 0>>>(
					&gpu_context->d_buf_score[batch_idx][0],
					&gpu_context->d_buf_softmax[batch_idx][0],
					&gpu_context->d_buf_expsum[batch_idx][0],
					seq_length, seq_length, num_heads);
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		// copy weight_VLW
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_VLW][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_VLW][0],
				hidden_size * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		// copy weight_VLB
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight[layer_idx][WEIGHT_VLB][0],
				&gpu_context->h_weight[layer_idx][WEIGHT_VLB][0],
				seq_length * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			float alpha = 1., beta = 0.;
			if ((cublas_rc = cublasSgemm(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
										 seq_length, head_size * num_heads, hidden_size,
										 &alpha, gpu_context->d_input[batch_idx], seq_length,
										 &gpu_context->d_weight[layer_idx][WEIGHT_VLW][0], hidden_size,
										 &beta, &gpu_context->d_buf_value[batch_idx][0], seq_length)) != CUBLAS_STATUS_SUCCESS) {
				fprintf(stderr, "[V_GEN] cublasSgemm (cublas_rc: %d)\n", cublas_rc); goto err;
			}

			if ((cublas_rc = cublasSaxpy(gpu_context->cublas_handles[0], seq_length * hidden_size, &alpha,
										 &gpu_context->d_weight[layer_idx][WEIGHT_VLB][0], 1,
										 &gpu_context->d_buf_value[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemmStridedBatched(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
													   ...
													   &alpha, &gpu_context->d_buf_softmax[batch_idx][0],
													   seq_length, seq_length * seq_length,
													   &gpu_context->d_buf_value[batch_idx][0],
													   seq_length, seq_length * head_size,
													   &beta, &gpu_context->d_buf_att[batch_idx][0],
													   seq_length, seq_length * head_size, num_heads)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_attention_fc_splitted[layer_idx][0],
				&gpu_context->h_weight_attention_fc_splitted[layer_idx][0],
				hidden_size * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_attention_fc_bias_splitted[layer_idx][0],
				&gpu_context->h_weight_attention_fc_bias_splitted[layer_idx][0],
				seq_length * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemmStridedBatched(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
													   ...
													   &alpha, &gpu_context->d_buf_att[batch_idx][0],
													   seq_length, seq_length * head_size,
													   &gpu_context->d_weight_attention_fc_splitted[layer_idx][0],
													   head_size, head_size * hidden_size,
													   &beta, &gpu_context->d_buf_att_fc_result_split[batch_idx][0],
													   seq_length, seq_length * hidden_size, num_heads)) != CUBLAS_STATUS_SUCCESS) {
		...
		/////////////
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			for (int head_idx = 1; head_idx < num_heads; head_idx++) {
				float alpha = 1.;
				if ((cublas_rc = cublasSaxpy(gpu_context->cublas_handles[0], seq_length * hidden_size, &alpha,
											 &gpu_context->d_buf_att_fc_result_split[batch_idx][head_idx * seq_length * hidden_size], 1,
											 &gpu_context->d_buf_att_fc_result_split[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		/// Do calculation
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			if (cuda_layer_norm(batch_idx, params->thread_block_size,
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_ffw_prev[layer_idx][0],
				&gpu_context->h_weight_ffw_prev[layer_idx][0],
				hidden_size * feedforward_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
			fprintf(stderr, "[Feedforward_PREV] <weight_FFW_PREV> cudaMemcpy (cuda_rc: %d)\n", cuda_rc);
			goto err;
		}
		// copy weight_FFW_PREV_BIAS
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_ffw_prev_bias[layer_idx][0],
				&gpu_context->h_weight_ffw_prev_bias[layer_idx][0],
				seq_length * feedforward_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			float alpha = 1., beta = 0.;
			if ((cublas_rc = cublasSgemm(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
										 seq_length, feedforward_size, hidden_size,
										 &alpha, gpu_context->d_buf_att_layernorm[batch_idx], seq_length,
										 &gpu_context->d_weight_ffw_prev[layer_idx][0], hidden_size,
										 &beta, &gpu_context->d_buf_ffw_intermediate[batch_idx][0], seq_length)) != CUBLAS_STATUS_SUCCESS) {
				fprintf(stderr, "[Feedforward_PREV] cublasSgemm (cublas_rc: %d)\n", cublas_rc); goto err;
			}
			if ((cublas_rc = cublasSaxpy(gpu_context->cublas_handles[0], seq_length * feedforward_size, &alpha,
										 &gpu_context->d_weight_ffw_prev_bias[layer_idx][0], 1,
										 &gpu_context->d_buf_ffw_intermediate[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			dim3 dimBlock(params->thread_block_size);
			dim3 dimGrid(seq_length * feedforward_size / params->thread_block_size);

			// gelu
			g_gelu<<<dimGrid, dimBlock, 0>>>(
					&gpu_context->d_buf_ffw_intermediate[batch_idx][0],
					&gpu_context->d_buf_ffw_gelu[batch_idx][0],
					seq_length * feedforward_size);
		}
//		sync_all_buf_to_host(bert_state, gpu_context);
		...
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_ffw_post_splitted[layer_idx][0],
				&gpu_context->h_weight_ffw_post_splitted[layer_idx][0],
				feedforward_size * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
		...
		// cooperate between streams to load the bias value (since we use this bias values at reduce_sum only)
		if ((cuda_rc = cudaMemcpy(
				&gpu_context->d_weight_ffw_post_bias_splitted[layer_idx][0],
				&gpu_context->h_weight_ffw_post_bias_splitted[layer_idx][0],
				seq_length * hidden_size * sizeof(float), cudaMemcpyHostToDevice)) != cudaSuccess) {
			fprintf(stderr, "[Feedforward_POST] <weight_FFW_POST_BIAS> cudaMemcpy (cuda_rc: %d)\n", cuda_rc);
			goto err;
		}
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			...
			if ((cublas_rc = cublasSgemmStridedBatched(gpu_context->cublas_handles[0], CUBLAS_OP_N, CUBLAS_OP_N,
													   ...
													   &alpha, &gpu_context->d_buf_ffw_gelu[batch_idx][0],
													   seq_length, seq_length * ffw_chunk_size,
													   &gpu_context->d_weight_ffw_post_splitted[layer_idx][0],
													   ffw_chunk_size, ffw_chunk_size * hidden_size,
													   &beta, &gpu_context->d_buf_ffw_result_split[batch_idx][0],
													   seq_length, seq_length * hidden_size, num_heads)) != CUBLAS_STATUS_SUCCESS) {
		...
		/////////////
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			for (int ffw_chunk_idx = 1; ffw_chunk_idx < num_heads; ffw_chunk_idx++) {
				float alpha = 1.;
				if ((cublas_rc = cublasSaxpy(gpu_context->cublas_handles[0], seq_length * hidden_size, &alpha,
											 &gpu_context->d_buf_ffw_result_split[batch_idx][ffw_chunk_idx * seq_length * hidden_size], 1,
											 &gpu_context->d_buf_ffw_result_split[batch_idx][0], 1)) != CUBLAS_STATUS_SUCCESS) {
...
//		/// Do calculation
		...
		for (int batch_idx = 0; batch_idx < num_batch; batch_idx++) {
			if ((cuda_rc = cudaMemcpy(gpu_context->d_input[batch_idx],
				...
				fprintf(stderr, "[Feedforward_NORM] <Final output> cudaMemcpy (cuda_rc: %d)\n", cuda_rc);
				goto err;
			}
		}
		cudaDeviceSynchronize();
...
void cuda_context_deinit(BERT_State *bert_state, gpu_cuda_context_t *gpu_context) {
	...
	cublasDestroy(*gpu_context->cublas_handles);
