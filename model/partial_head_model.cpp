...
void self_attention_partial_head::init_weight_splitted(BERT_State *bert_state, int layer_idx) {
...
self_attention_partial_head::self_attention_partial_head(Params *_params, BERT_State *_bert_state, int layer_idx, bool _is_training) {
	params = _params;
...
void self_attention_partial_head::create_attention_mask(float *attention_mask, float *mask) {
	vsSub(seq_length * seq_length, one_mat, attention_mask, mask);
	cblas_sscal(seq_length * seq_length, 10000.0, mask, 1);
}

void self_attention_partial_head::dump_values() {
...
float *self_attention_partial_head::forward(int batch_tid, float *input, float *attention_mask, int batch_idx, int layer_idx, Logger *loggers) {
...
static void *attention_threads_partial_head(void *_state) {
...
void self_attention_partial_head::self_attention_deinit() {
...
transformer_encoder_partial_head::transformer_encoder_partial_head(Params *_params, BERT_State *bert_state, int layer_idx, bool _is_training) {
	params = _params;
	is_training = _is_training;
	batch = bert_state->num_batch;
	num_heads = bert_state->num_heads;
	...
	attention_layer = new self_attention_partial_head(params, bert_state, layer_idx, is_training);
...
void transformer_encoder_partial_head::dump_values() {
...
float *transformer_encoder_partial_head::forward(int batch_tid, float *a, float *attention_mask, int batch_idx, int layer_idx, Logger *loggers) {
	Logger &logger = loggers[Logger::get_logger_idx(batch_idx, -1)];
...
void transformer_encoder_partial_head::transformer_encoder_deinit() {
...
BERT::BERT(Params *_params, BERT_State *_bert_state, bool _is_training, bool use_onehot, bool use_token_type, bool use_position_embedding) {
	...
	//encoder stack
	transformer_layer = (transformer_encoder_partial_head**)mkl_malloc(num_layers * sizeof(transformer_encoder_partial_head*), 64);

	for (int i = 0; i < num_layers; i++) {
		transformer_layer[i] = new transformer_encoder_partial_head(params, bert_state, i, is_training);
	}
...
float *BERT::forward(int **input_ids, float **input_mask, int **token_type_ids, Logger *loggers) {
	...
	for(int batch_tid = 0; batch_tid < batch_thread_num; batch_tid++) {
		...
		for (int head_tid = 0; head_tid < head_thread_num; head_tid++) {
			if (pthread_create(&att_th_list[batch_tid][head_tid], nullptr, attention_threads_partial_head,
