#ifndef MODEL_ALL_OPT_MODEL_HPP
#define MODEL_ALL_OPT_MODEL_HPP
...
//////////////////////
/// self_attention_all_opt
//////////////////////
class self_attention_all_opt{
private:
	Params *params;
...
public:
	self_attention_all_opt(Params *_params, BERT_State *bert_state, int layer_idx, bool _is_training);
	void dump_values();
...
//////////////////////
/// transformer_encoder_all_opt
//////////////////////
class transformer_encoder_all_opt{
private:
	Params *params;
	BERT_State *bert_state;
	bool is_training;
	...
	self_attention_all_opt *attention_layer;
	...
	/// [head][weight (head_size x hidden_size)]
	float **weight_att_fc_splitted;
	/// [head][weight (Hidden x FF_SIZE/heads)]
	float **weight_ffw_prev_splitted;
	/// [head][weight (FF_SIZE/heads x Hidden)]
	float **weight_ffw_post_splitted;

	/// [head][weight (1 x hidden_size)]
	float **bias_att_fc_splitted;
	/// [head][weight (1 x FF_SIZE/heads)]
	float **bias_ffw_prev_splitted;
	/// [head][weight (1 x Hidden)]
	float **bias_ffw_post_splitted;

	float **context;
	float ***head_split_context;
	float ***attention_result_split_tmp;
	...
	float ***feedforward_result_split_tmp;
	float **feedforward_result;
	float **feedforward_residual;
	float **feedforward_layernorm;

	void init_ffw_weight_splitted(BERT_State *bert_state, int layer_idx);

public:
	transformer_encoder_all_opt(Params *_params, BERT_State *bert_state, int layer_idx, bool _is_training);
	void dump_values();
...
private:
	...
	embedding *embedding_layer;
	transformer_encoder_all_opt **transformer_layer;
	layer_dense *pooler;
