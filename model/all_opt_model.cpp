...
void self_attention_all_opt::init_weight_splitted(BERT_State *bert_state, int layer_idx) {
...
self_attention_all_opt::self_attention_all_opt(Params *_params, BERT_State *_bert_state, int layer_idx, bool _is_training) {
	params = _params;
...
void self_attention_all_opt::create_attention_mask(float *attention_mask, float *mask) {
	vsSub(seq_length * seq_length, one_mat, attention_mask, mask);
	cblas_sscal(seq_length * seq_length, 10000.0, mask, 1);
}

void self_attention_all_opt::dump_values() {
...
float *self_attention_all_opt::forward(int batch_tid, float *input, float *attention_mask, int batch_idx, int layer_idx, Logger *loggers) {
	Logger &logger = loggers[Logger::get_logger_idx(batch_idx, -1, true)];
...
static void *attention_threads_all_opt(void *_state) {
	...
	/// Cache init
	Logger::init_csmon(batch_tid, head_tid, false, true);	// Only track itself
	Logger::csmon_start(batch_tid, head_tid, true);
	...
	while (true) {
		...
		for (int head_idx = head_start_idx;  head_idx < head_start_idx + num_head_per_thread; head_idx++) {
			Logger &logger = loggers[Logger::get_logger_idx(batch_idx, head_idx, true)];
			logger.set_tids(batch_tid, head_tid);
			logger.set_cs_counter(Logger::get_csmon(batch_tid, head_tid, true)->get_counter());
	...
	Logger::csmon_stop(batch_tid, head_tid, true);
	Logger::deinit_csmon(batch_tid, head_tid, true);

	return nullptr;
}

void self_attention_all_opt::self_attention_deinit() {
...
void transformer_encoder_all_opt::init_ffw_weight_splitted(BERT_State *bert_state, int layer_idx) {
	weight_att_fc_splitted = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	weight_ffw_prev_splitted = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	weight_ffw_post_splitted = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	bias_att_fc_splitted = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	bias_ffw_prev_splitted= (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	bias_ffw_post_splitted = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
	for (int h = 0; h < num_heads; h++) {
		weight_att_fc_splitted[h] = (float*)mkl_calloc(head_size, hidden_size * sizeof(float), 64);
		weight_ffw_prev_splitted[h] = (float*)mkl_calloc(hidden_size, ffw_chunk_size * sizeof(float), 64);
		weight_ffw_post_splitted[h] = (float*)mkl_calloc(ffw_chunk_size, hidden_size * sizeof(float), 64);
		bias_att_fc_splitted[h] = (float*)mkl_calloc(1, hidden_size * sizeof(float), 64);
		bias_ffw_prev_splitted[h] = (float*)mkl_calloc(1, ffw_chunk_size * sizeof(float), 64);
		bias_ffw_post_splitted[h] = (float*)mkl_calloc(1, hidden_size * sizeof(float), 64);
	}

	for (int h = 0; h < num_heads; h++) {
		for (int k = 0; k < hidden_size; k++) {
			memcpy(&weight_ffw_prev_splitted[h][k * ffw_chunk_size], &bert_state->weight[layer_idx][WEIGHT_PREV_FFW][k * feedforward_size + h * ffw_chunk_size], ffw_chunk_size * sizeof(float));
		}
		memcpy(bias_ffw_prev_splitted[h], &bert_state->weight[layer_idx][WEIGHT_PREV_FFB][h * ffw_chunk_size], ffw_chunk_size * sizeof(float));
	}

	for (int h = 0; h < num_heads; h++) {
		memcpy(weight_att_fc_splitted[h], &bert_state->weight[layer_idx][WEIGHT_ATTENTION][h * head_size * hidden_size], head_size * hidden_size * sizeof(float));
		memcpy(weight_ffw_post_splitted[h], &bert_state->weight[layer_idx][WEIGHT_POST_FFW][h * ffw_chunk_size * hidden_size], ffw_chunk_size * hidden_size * sizeof(float));
		memcpy(bias_att_fc_splitted[h], bert_state->weight[layer_idx][WEIGHT_ATTENTION_BIAS], hidden_size * sizeof(float));
		memcpy(bias_ffw_post_splitted[h], bert_state->weight[layer_idx][WEIGHT_POST_FFB], hidden_size * sizeof(float));
	}
}

transformer_encoder_all_opt::transformer_encoder_all_opt(Params *_params, BERT_State *_bert_state, int layer_idx, bool _is_training) {
	params = _params;
	bert_state = _bert_state;
	is_training = _is_training;
	batch = bert_state->num_batch;
	...
	seq_length = bert_state->seq_length;
	hidden_size = bert_state->hidden_size;
	head_size = bert_state->hidden_size / bert_state->num_heads;
	feedforward_size = bert_state->feedforwardsize;
	ffw_chunk_size = bert_state->feedforwardsize / bert_state->num_heads;
	...
	pthread_barrier_init(barrier_tfenc_attention_fc, nullptr, num_th_head * params->num_th_batch);
	...
	pthread_barrier_init(barrier_tfenc_feedforward_pre, nullptr, num_th_head * params->num_th_batch);
	pthread_barrier_init(barrier_tfenc_feedforward_gelu, nullptr, num_th_head * params->num_th_batch);
	pthread_barrier_init(barrier_tfenc_feedforward_post, nullptr, num_th_head * params->num_th_batch);
	...
	/// Init FFW (PREV/POST) weight and bias
	init_ffw_weight_splitted(bert_state, layer_idx);

	attention_layer = new self_attention_all_opt(params, bert_state, layer_idx, is_training);
	...
	attention_fc = (layer_dense *)mkl_calloc(num_heads, sizeof(layer_dense), 64);
	prev_feedforward = (layer_dense *)mkl_calloc(num_heads, sizeof(layer_dense), 64);
	gelu_feedforward= (gelu *)mkl_calloc(num_heads, sizeof(gelu), 64);
	post_feedforward = (layer_dense *)mkl_calloc(num_heads, sizeof(layer_dense), 64);
	for (int h = 0; h < num_heads; h++) {
		attention_fc[h] = layer_dense(weight_att_fc_splitted[h], bias_att_fc_splitted[h],
		                    is_training, seq_length, hidden_size, head_size);
		prev_feedforward[h] = layer_dense(weight_ffw_prev_splitted[h], bias_ffw_prev_splitted[h],
		                    is_training, seq_length, ffw_chunk_size, hidden_size);
		gelu_feedforward[h] = gelu(is_training, batch, seq_length * ffw_chunk_size);
		post_feedforward[h] = layer_dense(weight_ffw_post_splitted[h], bias_ffw_post_splitted[h],
		                    is_training, seq_length, hidden_size, ffw_chunk_size);
	}
	...
	head_split_context=(float ***)mkl_calloc(batch, sizeof(float**), 64);
	attention_result_split_tmp=(float ***)mkl_calloc(batch, sizeof(float**), 64);
	...
	feedforward_result_split_tmp=(float ***)mkl_calloc(batch, sizeof(float**), 64);
	...
	for(int i=0; i<batch; i++){
		head_split_context[i] = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
		attention_result_split_tmp[i] = (float **)mkl_calloc(num_heads, sizeof(float*), 64);
		for (int h = 0; h < num_heads; h++) {
			head_split_context[i][h] = (float *)mkl_calloc(seq_length, head_size * sizeof(float), 64);
			attention_result_split_tmp[i][h] = (float *)mkl_calloc(seq_length, hidden_size * sizeof(float), 64);
			memset(head_split_context[i][h],         0, sizeof(float) * seq_length * head_size);
			memset(attention_result_split_tmp[i][h], 0, sizeof(float) * seq_length * hidden_size);
		}
		...
		feedforward_intermediate[i] =(float **)mkl_calloc(num_heads, sizeof(float*), 64);
		feedforward_gelu[i] =(float **)mkl_calloc(num_heads, sizeof(float*), 64);
		feedforward_result_split_tmp[i] =(float **)mkl_calloc(num_heads, sizeof(float*), 64);

		for (int ffw_chunk_idx = 0;  ffw_chunk_idx < num_heads; ffw_chunk_idx++) {
			feedforward_intermediate[i][ffw_chunk_idx] = (float*)mkl_calloc(seq_length, ffw_chunk_size * sizeof(float), 64);
			feedforward_gelu[i][ffw_chunk_idx] = (float*)mkl_calloc(seq_length, ffw_chunk_size * sizeof(float), 64);
			feedforward_result_split_tmp[i][ffw_chunk_idx] = (float*)mkl_calloc(seq_length, hidden_size * sizeof(float), 64);

			memset(feedforward_intermediate[i][ffw_chunk_idx],     0, sizeof(float) * seq_length * ffw_chunk_size);
			memset(feedforward_gelu[i][ffw_chunk_idx],             0, sizeof(float) * seq_length * ffw_chunk_size);
			memset(feedforward_result_split_tmp[i][ffw_chunk_idx], 0, sizeof(float) * seq_length * hidden_size);
		}
	}
}

void transformer_encoder_all_opt::dump_values() {
	...
	for (int batch_idx = 0; batch_idx < batch; batch_idx++) {
		...
		printf("feedforward_intermediate[%d][0] => ", batch_idx);
		for (int i = 0; i < 16; i++) { printf("%f ", feedforward_intermediate[batch_idx][0][i]); } printf("\n");
		printf("feedforward_gelu[%d][0] => ", batch_idx);
		for (int i = 0; i < 16; i++) { printf("%f ", feedforward_gelu[batch_idx][0][i]); } printf("\n");
...
float *transformer_encoder_all_opt::forward(int batch_tid, float *a, float *attention_mask, int batch_idx, int layer_idx, Logger *loggers) {
	Logger &logger = loggers[Logger::get_logger_idx(batch_idx, -1, true)];
	...
	/// data split for partial FC
	for (int s = 0; s < seq_length; s++) {
		for (int h = 0; h < num_heads; h++) {
			memcpy(&head_split_context[batch_idx][h][s * head_size], &context[batch_idx][s * hidden_size + h * head_size], head_size * sizeof(float));
		}
	}
	/// Setting parameters for ffw_worker threads
	for (int head_tid = 0; head_tid < num_th_head; head_tid++) {
		Feedforward_State *ffw_st = bert_state->ffw_st_list[batch_tid * num_th_head + head_tid];
		ffw_st->attention_fc = attention_fc;

		/// buffer
		ffw_st->attention_result_split_tmp = attention_result_split_tmp;

		/// barrier
		ffw_st->barrier_tfenc_attention_fc = barrier_tfenc_attention_fc;

		/// input & indexes
		ffw_st->head_split_context = head_split_context;
		ffw_st->layer_idx = layer_idx;
		ffw_st->batch_idx = batch_idx;
	}

	/// Send a starting signal to workers
	pthread_barrier_wait(bert_state->barrier_ffw_start);

	/////////////
	/// [Waiting for] Attention_FC
	/////////////

	/// Waiting for attention workers
	pthread_barrier_wait(bert_state->barrier_ffw_end);

	/////////////
	/// Attention_FC Sum (FIXME: doesn't it require latency & cachestat measurement?)
	/// *** NOTE that we reduce bias for each reduce_sum because bias addition should be only applied once.
	///    We manually subtract multiple bias values HERE because we want to include the reduce_sum overhead in the evaluation.
	/////////////
	memset(attention_result[batch_idx], 0, sizeof(float) * seq_length * hidden_size);
	// Aggregate all results into feedforward_result
	for (int head_idx = 0; head_idx < num_heads; head_idx++) {
		vsAdd(seq_length * hidden_size, attention_result[batch_idx],attention_result_split_tmp[batch_idx][head_idx],
		      attention_result[batch_idx]);
		if (head_idx != 0) {
			for (int s = 0; s < seq_length; s++) {
				vsSub(hidden_size, &attention_result[batch_idx][s * hidden_size], attention_fc[head_idx].b_add.bias,
				      &attention_result[batch_idx][s * hidden_size]);
			}
		}
	}
	...
	for (int head_tid = 0; head_tid < num_th_head; head_tid++) {
		Feedforward_State *ffw_st = bert_state->ffw_st_list[batch_tid * num_th_head + head_tid];
		ffw_st->prev_feedforward = prev_feedforward;
		ffw_st->gelu_feedforward = gelu_feedforward;
		ffw_st->post_feedforward = post_feedforward;

		ffw_st->feedforward_intermediate = feedforward_intermediate;
		ffw_st->feedforward_gelu = feedforward_gelu;
		ffw_st->feedforward_result_split_tmp = feedforward_result_split_tmp;

		ffw_st->barrier_tfenc_feedforward_pre = barrier_tfenc_feedforward_pre;
		ffw_st->barrier_tfenc_feedforward_gelu = barrier_tfenc_feedforward_gelu;
		ffw_st->barrier_tfenc_feedforward_post = barrier_tfenc_feedforward_post;

		ffw_st->attention_layernorm = attention_layernorm;
		ffw_st->layer_idx = layer_idx;
		ffw_st->batch_idx = batch_idx;
	}
	...
	/////////////
	/// [Waiting for] FFW_PREV
	/////////////

	/////////////
	/// [Waiting for] FFW_GELU
	/////////////

	/////////////
	/// [Waiting for] FFW_POST
	/////////////
	...
	/////////////
	/// FFW_Reduce Sum (FIXME: doesn't it require latency & cachestat measurement?)
	/// *** NOTE that we reduce bias for each reduce_sum because bias addition should be only applied once.
	///    We manually subtract multiple bias values HERE because we want to include the reduce_sum overhead in the evaluation.
	/////////////
	memset(feedforward_result[batch_idx], 0, sizeof(float) * seq_length * hidden_size);
	// Aggregate all results into feedforward_result
	for (int ffw_chunk_idx = 0; ffw_chunk_idx < num_heads; ffw_chunk_idx++) {
		vsAdd(seq_length * hidden_size, feedforward_result[batch_idx],feedforward_result_split_tmp[batch_idx][ffw_chunk_idx],
			  feedforward_result[batch_idx]);
		if (ffw_chunk_idx != 0) {
			for (int s = 0; s < seq_length; s++) {
				vsSub(hidden_size, &feedforward_result[batch_idx][s * hidden_size], post_feedforward[ffw_chunk_idx].b_add.bias,
				      &feedforward_result[batch_idx][s * hidden_size]);
			}
		}
	}
...
static void *ffw_threads_all_opt(void *_state) {
	Feedforward_State *ffw_st = (Feedforward_State*)_state;
	const EXEC_MODE exec_mode = ffw_st->exec_mode;
	const int batch_tid = ffw_st->batch_tid;
	const int head_tid = ffw_st->head_tid;
	const int num_batchs = ffw_st->num_batchs;
	const int batch_th_num = ffw_st->batch_th_num;
	const int num_batch_per_thread = num_batchs / batch_th_num;
	const int head_th_num = ffw_st->head_th_num;
	const int num_heads = ffw_st->num_heads;
	const int num_head_per_thread = num_heads / head_th_num;
	const int num_ffw_chunks_per_thread = num_heads / head_th_num; // FIXME: feedforward should be independently chunked, not based on # of heads
	const int num_layers = ffw_st->num_layers;
	const MKL_INT seq_length = ffw_st->seq_length;
	const MKL_INT hidden_size = ffw_st->hidden_size;
	const MKL_INT head_size = ffw_st->head_size;
	const int head_start_idx = head_tid * num_head_per_thread;
	const MKL_INT feedforward_size = ffw_st->feedforward_size;
	const MKL_INT ffw_chunk_size = ffw_st->ffw_chunk_size;
	const int ffw_chunk_start_idx = head_tid * num_ffw_chunks_per_thread;

	Logger *loggers = ffw_st->loggers;

	layer_dense *prev_feedforward = nullptr;
	layer_dense *attention_fc = nullptr;
	gelu *gelu_feedforward = nullptr;
	layer_dense *post_feedforward = nullptr;

	float ***attention_result_split_tmp = nullptr;
	float ***feedforward_intermediate = nullptr;
	float ***feedforward_gelu = nullptr;
	float ***feedforward_result_split_tmp = nullptr;

	pthread_barrier_t *barrier_tfenc_attention_fc = nullptr;
	pthread_barrier_t *barrier_tfenc_feedforward_pre = nullptr;
	pthread_barrier_t *barrier_tfenc_feedforward_gelu = nullptr;
	pthread_barrier_t *barrier_tfenc_feedforward_post = nullptr;

	float **attention_layernorm = nullptr;
	float **context = nullptr;
	float ***head_split_context = nullptr;
	int layer_idx = -1;
	int batch_idx = -1;
	...
	mkl_set_num_threads_local(1);
	/// Cache init
	Logger::init_csmon(batch_tid, head_tid + head_th_num, false, true);	// Only track itself
	Logger::csmon_start(batch_tid, head_tid + head_th_num, true);
	...
	while (true) {
		...
		/////////////////// Phase 1 (Attention_FC)
		pthread_barrier_wait(ffw_st->barrier_ffw_start);
		attention_fc = ffw_st->attention_fc;

		attention_result_split_tmp = (float***)ffw_st->attention_result_split_tmp;

		barrier_tfenc_attention_fc = ffw_st->barrier_tfenc_attention_fc;

		head_split_context = ffw_st->head_split_context;
		layer_idx = ffw_st->layer_idx;
		batch_idx = ffw_st->batch_idx;

		for (int head_idx = head_start_idx;  head_idx < head_start_idx + num_head_per_thread; head_idx++) {
			Logger &logger = loggers[Logger::get_logger_idx(batch_idx, head_idx + num_heads, true)]; // FIXME: feedforward should be independently chunked, not based on # of heads
			logger.set_tids(batch_tid, head_tid);
			logger.set_cs_counter(Logger::get_csmon(batch_tid, head_tid + head_th_num, true)->get_counter());

			/////////////
			/// Attention_FC
			/////////////
			flush_cache(head_split_context[batch_idx][head_idx], seq_length * head_size * sizeof(float));
			flush_cache(attention_fc[head_idx].weight, head_size * hidden_size * sizeof(float));
			flush_cache(attention_fc[head_idx].b_add.bias, 1 * hidden_size * sizeof(float));
			flush_cache(attention_result_split_tmp[batch_idx][head_idx], seq_length * hidden_size * sizeof(float));
			prefetch_data(head_split_context[batch_idx][head_idx], seq_length * head_size * sizeof(float));
			prefetch_data(attention_fc[head_idx].weight, head_size * hidden_size * sizeof(float));
			prefetch_data(attention_fc[head_idx].b_add.bias, 1 * hidden_size * sizeof(float));
			prefetch_data(attention_result_split_tmp[batch_idx][head_idx], seq_length * hidden_size * sizeof(float));
			pthread_barrier_wait(barrier_tfenc_attention_fc);
			logger.infer_logging_begin(ATTENTION_FC, layer_idx);
			attention_fc[head_idx].forward(head_split_context[batch_idx][head_idx], CblasNoTrans, attention_result_split_tmp[batch_idx][head_idx]);
			logger.infer_logging_end(ATTENTION_FC, layer_idx);
		}
		/// Send a signal that the calculation is finished
		pthread_barrier_wait(ffw_st->barrier_ffw_end);


		/////////////////// Phase 2 (Feedforward_Prev & GELU & Feedforward_Post)
		pthread_barrier_wait(ffw_st->barrier_ffw_start);
		prev_feedforward = ffw_st->prev_feedforward;
		gelu_feedforward = ffw_st->gelu_feedforward;
		post_feedforward = ffw_st->post_feedforward;


		feedforward_intermediate = (float***)ffw_st->feedforward_intermediate;
		feedforward_gelu = (float***)ffw_st->feedforward_gelu;
		feedforward_result_split_tmp = (float***)ffw_st->feedforward_result_split_tmp;

		barrier_tfenc_feedforward_pre = ffw_st->barrier_tfenc_feedforward_pre;
		barrier_tfenc_feedforward_gelu = ffw_st->barrier_tfenc_feedforward_gelu;
		barrier_tfenc_feedforward_post = ffw_st->barrier_tfenc_feedforward_post;

		attention_layernorm = ffw_st->attention_layernorm;
		layer_idx = ffw_st->layer_idx;
		batch_idx = ffw_st->batch_idx;

		for (int ffw_chunk_idx = ffw_chunk_start_idx;  ffw_chunk_idx < ffw_chunk_start_idx + num_ffw_chunks_per_thread; ffw_chunk_idx++) {
			Logger &logger = loggers[Logger::get_logger_idx(batch_idx, ffw_chunk_idx + num_heads, true)]; // FIXME: feedforward should be independently chunked, not based on # of heads

			/////////////
			/// FFW_PREV
			/////////////
			flush_cache(attention_layernorm[batch_idx], seq_length * hidden_size * sizeof(float));
			flush_cache(prev_feedforward[ffw_chunk_idx].weight, hidden_size * ffw_chunk_size * sizeof(float));
			flush_cache(prev_feedforward[ffw_chunk_idx].b_add.bias, 1 * ffw_chunk_size * sizeof(float));
			flush_cache(feedforward_intermediate[batch_idx][ffw_chunk_idx], seq_length * ffw_chunk_size * sizeof(float));
			prefetch_data(attention_layernorm[batch_idx], seq_length * hidden_size * sizeof(float));
			prefetch_data(prev_feedforward[ffw_chunk_idx].weight, hidden_size * ffw_chunk_size * sizeof(float));
			prefetch_data(prev_feedforward[ffw_chunk_idx].b_add.bias, 1 * ffw_chunk_size * sizeof(float));
			prefetch_data(feedforward_intermediate[batch_idx][ffw_chunk_idx], seq_length * ffw_chunk_size * sizeof(float));
			pthread_barrier_wait(barrier_tfenc_feedforward_pre);
			logger.infer_logging_begin(FEEDFORWARD_PRE, layer_idx);
			prev_feedforward[ffw_chunk_idx].forward(attention_layernorm[batch_idx], CblasNoTrans, feedforward_intermediate[batch_idx][ffw_chunk_idx]);
			...
			/// FFW_GELU
			/////////////
			assert(gelu_feedforward[ffw_chunk_idx].size == seq_length * ffw_chunk_size);
			flush_cache(feedforward_intermediate[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			flush_cache(gelu_feedforward[ffw_chunk_idx].temp[batch_idx], sizeof(float) * seq_length * ffw_chunk_size);
			flush_cache(gelu_feedforward[ffw_chunk_idx].temp1[batch_idx], sizeof(float) * seq_length * ffw_chunk_size);
			flush_cache(feedforward_gelu[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			prefetch_data(feedforward_intermediate[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			prefetch_data(gelu_feedforward[ffw_chunk_idx].temp[batch_idx], sizeof(float) * seq_length * ffw_chunk_size);
			prefetch_data(gelu_feedforward[ffw_chunk_idx].temp1[batch_idx], sizeof(float) * seq_length * ffw_chunk_size);
			prefetch_data(feedforward_gelu[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			pthread_barrier_wait(barrier_tfenc_feedforward_gelu);
			logger.infer_logging_begin(FEEDFORWARD_GELU, layer_idx);
			gelu_feedforward[ffw_chunk_idx].forward(feedforward_intermediate[batch_idx][ffw_chunk_idx], batch_idx, feedforward_gelu[batch_idx][ffw_chunk_idx]);
			...
			/////////////
			flush_cache(feedforward_gelu[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			flush_cache(post_feedforward[ffw_chunk_idx].weight, ffw_chunk_size * hidden_size * sizeof(float));
			flush_cache(post_feedforward[ffw_chunk_idx].b_add.bias, 1 * hidden_size * sizeof(float));
			flush_cache(feedforward_result_split_tmp[batch_idx][ffw_chunk_idx], seq_length * hidden_size * sizeof(float));
			prefetch_data(feedforward_gelu[batch_idx][ffw_chunk_idx], sizeof(float) * seq_length * ffw_chunk_size);
			prefetch_data(post_feedforward[ffw_chunk_idx].weight, ffw_chunk_size * hidden_size * sizeof(float));
			prefetch_data(post_feedforward[ffw_chunk_idx].b_add.bias, 1 * hidden_size * sizeof(float));
			prefetch_data(feedforward_result_split_tmp[batch_idx][ffw_chunk_idx], seq_length * hidden_size * sizeof(float));
			pthread_barrier_wait(barrier_tfenc_feedforward_post);
			logger.infer_logging_begin(FEEDFORWARD_POST, layer_idx);
			post_feedforward[ffw_chunk_idx].forward(feedforward_gelu[batch_idx][ffw_chunk_idx], CblasNoTrans, feedforward_result_split_tmp[batch_idx][ffw_chunk_idx]);
	...
	Logger::csmon_stop(batch_tid, head_tid + head_th_num, true);
	Logger::deinit_csmon(batch_tid, head_tid + head_th_num, true);

	return nullptr;
}

void transformer_encoder_all_opt::transformer_encoder_deinit() {
	...
	for (int h = 0; h < num_heads; h++) {
		gelu_feedforward[h].gelu_deinit();
	}
	mkl_free(attention_fc);
	mkl_free(prev_feedforward);
	mkl_free(gelu_feedforward);
	mkl_free(post_feedforward);

	feedforward_norm.layer_norm_deinit();

	for (int ffw_chunk_idx = 0;  ffw_chunk_idx < num_heads; ffw_chunk_idx++) {
		mkl_free(weight_att_fc_splitted[ffw_chunk_idx]);
		mkl_free(weight_ffw_prev_splitted[ffw_chunk_idx]);
		mkl_free(weight_ffw_post_splitted[ffw_chunk_idx]);
		mkl_free(bias_att_fc_splitted[ffw_chunk_idx]);
		mkl_free(bias_ffw_prev_splitted[ffw_chunk_idx]);
		mkl_free(bias_ffw_post_splitted[ffw_chunk_idx]);
	}
	mkl_free(weight_att_fc_splitted);
	mkl_free(weight_ffw_prev_splitted);
	mkl_free(weight_ffw_post_splitted);
	mkl_free(bias_att_fc_splitted);
	mkl_free(bias_ffw_prev_splitted);
	mkl_free(bias_ffw_post_splitted);

	for(int i=0; i<batch; i++){
		for (int h = 0; h < num_heads; h++) {
			mkl_free(head_split_context[i][h]);
			mkl_free(attention_result_split_tmp[i][h]);
		}
		mkl_free(head_split_context[i]);
		mkl_free(attention_result_split_tmp[i]);

		for (int ffw_chunk_idx = 0;  ffw_chunk_idx < num_heads; ffw_chunk_idx++) {
			mkl_free(feedforward_intermediate[i][ffw_chunk_idx]);
			mkl_free(feedforward_gelu[i][ffw_chunk_idx]);
			mkl_free(feedforward_result_split_tmp[i][ffw_chunk_idx]);
		}
		...
		mkl_free(feedforward_result_split_tmp[i]);
		mkl_free(feedforward_result[i]);
	}
	mkl_free(head_split_context); head_split_context = nullptr;
	mkl_free(attention_result_split_tmp); attention_result_split_tmp = nullptr;
	...
	mkl_free(feedforward_result_split_tmp); feedforward_result_split_tmp = nullptr;
...
BERT::BERT(Params *_params, BERT_State *_bert_state, bool _is_training, bool use_onehot, bool use_token_type, bool use_position_embedding) {
	...
	//encoder stack
	transformer_layer = (transformer_encoder_all_opt**)mkl_malloc(num_layers * sizeof(transformer_encoder_all_opt*), 64);

	for (int i = 0; i < num_layers; i++) {
		transformer_layer[i] = new transformer_encoder_all_opt(params, bert_state, i, is_training);
	}
...
float *BERT::forward(int **input_ids, float **input_mask, int **token_type_ids, Logger *loggers) {
	...
	ffw_th_list = (pthread_t**)malloc(sizeof(pthread_t*) * batch_thread_num);
	...
	for(int batch_tid = 0; batch_tid < batch_thread_num; batch_tid++) {
		...
		for (int head_tid = 0; head_tid < head_thread_num; head_tid++) {
			if (pthread_create(&att_th_list[batch_tid][head_tid], nullptr, attention_threads_all_opt,
		...
		/// ffw threads for post processing after multi-head (except attention-FC)
		ffw_th_list[batch_tid] = new pthread_t[head_thread_num];
		for (int head_tid = 0; head_tid < head_thread_num; head_tid++) {
			if (pthread_create(&ffw_th_list[batch_tid][head_tid], nullptr, ffw_threads_all_opt,
			                   bert_state->ffw_st_list[batch_tid * params->num_th_head + head_tid]) != 0) {
				assert(0);
			}
			...
			CPU_ZERO(&cpuset);
			CPU_SET(batch_tid * params->num_th_head + head_tid, &cpuset);
			if (pthread_setaffinity_np(ffw_th_list[batch_tid][head_tid], sizeof(cpu_set_t), &cpuset) < 0) {
				assert(0);
			}
		}
	}
	for(int batch_tid = 0; batch_tid < batch_thread_num; batch_tid++) {
		for (int head_tid = 0; head_tid < head_thread_num; head_tid++) {
			if (pthread_join(att_th_list[batch_tid][head_tid], nullptr) != 0) { assert(0); }
			if (pthread_join(ffw_th_list[batch_tid][head_tid], nullptr) != 0) { assert(0); }
		}
		delete[] (att_th_list[batch_tid]);
		delete[] (ffw_th_list[batch_tid]);
...
void *BERT::forward_pth(int batch_tid, int id, int ** input, float ** mask, int ** token_type, float **t_output, Logger *loggers){
	...
	Logger::init_csmon(batch_tid, -1, true, true);		// count child threads (to track mkl threads)
	Logger::csmon_start(batch_tid, -1, true);

	for(int i=0; i<batch/batch_thread_num; i++){
		batch_idx = id + i;
		logger_idx = Logger::get_logger_idx(batch_idx, -1, true);
		loggers[logger_idx].set_tids(batch_tid, -1);
		loggers[logger_idx].set_cs_counter(Logger::get_csmon(batch_tid, -1, true)->get_counter());
	...
	Logger::csmon_stop(batch_tid, -1, true);
	Logger::deinit_csmon(batch_tid, -1, true);
	return nullptr;
}
